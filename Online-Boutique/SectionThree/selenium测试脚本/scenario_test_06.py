...
class PerformanceTester:
    ...
    def measure(self, action, label, time_list=None, save_home_time=False):
    ...
    def run_test(self):
        self.measure(lambda: self.driver.get("http://127.0.0.1:60462/"), "首页加载", save_home_time=True)
        self.driver.set_window_size(2576, 1408)

        self.measure(lambda: self.driver.find_element(By.CSS_SELECTOR, ".col-md-4:nth-child(2) .hot-product-card-img-overlay").click(), "点击商品卡片", self.interaction_times)
        self.measure(lambda: self.driver.find_element(By.CSS_SELECTOR, ".cymbal-button-primary").click(), "点击加入购物车", self.interaction_times)
        self.measure(lambda: self.driver.find_element(By.CSS_SELECTOR, ".col-md-3:nth-child(1) img").click(), "点击推荐商品图", self.interaction_times)
        self.measure(lambda: self.driver.find_element(By.CSS_SELECTOR, ".cymbal-button-primary").click(), "再次加入购物车", self.interaction_times)
